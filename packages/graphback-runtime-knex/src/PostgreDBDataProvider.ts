...
export class PostgreDBDataProvider<Type = any, GraphbackContext = any> implements GraphbackDataProvider<Type, GraphbackContext>{
  ...
  public async create(data: Type): Promise<Type> {
    ...
    const [id] = await this.db.query(`INSERT INTO ${this.tableName} VALUES (${createData})`);
    //tslint:disable-next-line: await-promise
    // const dbResult = await this.db.select().from(this.tableName).where(this.tableMap.idField, '=', id)
    const dbResult = await this.db.query(`SELECT * FROM ${this.tableName} WHERE ${this.tableMap.idField} =  ${id}`);
  ...
  public async update(data: Type): Promise<Type> {
    ...
    const updateResult = await this.db.query(`UPDATE ${this.tableName} WHERE ${idField.name} = ${idField.value}`);
    if (updateResult === 1) {
      //tslint:disable-next-line: await-promise
      // const dbResult = await this.db.select().from(this.tableName).where(idField.name, '=', idField.value);
      const dbResult = await this.db.query(`SELECT * FROM ${this.tableName} WHERE ${idField.name} = ${idField.value}`);
      if (dbResult && dbResult[0]) {
        return dbResult[0]
      }
    }
  ...
  public async delete(data: Type): Promise<Type> {
    ...
    const beforeDelete = await this.db.query(`SELECT * FROM ${this.tableName} WHERE ${idField.name} = ${idField.value}`)
    //tslint:disable-next-line: await-promise
    // const dbResult = await this.db(this.tableName).where(idField.name, '=', idField.value).del()
    const dbResult = await this.db.query(`DELETE FROM ${this.tableName} WHERE ${idField.name} = ${idField.value}`);
  ...
  public async findAll(page?: GraphbackPage): Promise<Type[]> {
    ...
    const dbResult = await this.db.query(this.usePage(query, page));
    if (dbResult) {
      return dbResult;
    }
  ...
  public async findBy(filter: Type | AdvancedFilter, page?: GraphbackPage): Promise<Type[]> {
    ...
    const query = `SELECT * FROM ${this.tableName} WHERE ${data}`; // this is not the correct implimentation for where clause
    const dbResult = await this.db.query(this.usePage(query, page));
    if (dbResult) {
      return dbResult;
    }
  ...
  public async batchRead(relationField: string, ids: string[]): Promise<Type[][]> {
    ...
    const dbResult = await this.db.query(`SELECT * FROM ${this.tableName} WHERE ${relationField} IN (${ids})`) // this is not the correct implimentation for where clause
  ...
  private usePage(query: String, page?: GraphbackPage, defaultLimit: number = 10, defaultOffset: number = 0) {
    if (page) {
